...
function Navbar() {
  ...
  const menuItems = [
    { path: '/profil', icon: <img src={LogoProfil} alt="Profil" className="w-6 h-6 rounded-full" />, label: 'Profil' },
    { path: '/dashboard', icon: <FaHome />, label: 'Dashboard' },
    { path: '/client', icon: <FaPeopleGroup />, label: 'Clients' },
    { path: '/prediction', icon: <FaChartLine />, label: 'Prédiction' },
    // { path: '/traitement', icon: <FaCog />, label: 'Traitement' },
  ...
  return (
    <>
      {/* Navbar Desktop avec margin-bottom ajouté */}
      <nav className={`fixed top-0 left-0 right-0 z-50 mb-18 transition-all duration-500 ${scrolled ? 'bg-gray-900/90 backdrop-blur-md py-2 shadow-xl' : 'bg-gray-900/80 backdrop-blur-sm py-3'}`}>
      ...
      {/* Espace pour le contenu sous la navbar - ajusté pour compenser le margin-bottom */}
      <div className="pt-20"></div> {/* Augmenté de 20px à 28px pour compenser le mb-8 */}
    </>
  );
}
