...
#     query = (
#         db.query(User.nom, func.sum(Consommation.valeur).label("total"))
...
#     results = (
#         query.group_by(User.nom)
#         .order_by(func.sum(Consommation.valeur).desc())
#         .all()
#     )

#     # Pas plus de 8 clients dans la liste
#     if len(results) <= 8:
#         return [{"client": nom, "consommation": total} for nom, total in results]

#     # Sinon : top 8 + "Autres"
#     top_8 = results[:8]
#     autres_total = sum(row.total for row in results[8:])

#     data = [{"client": nom, "consommation": total} for nom, total in top_8]
...
#     top_8 = results[:8]
#     autres_total = sum(row[1] for row in results[8:])  # ✅ Correction ici
